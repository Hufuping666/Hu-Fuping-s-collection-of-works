...
void yi()  //双人战斗 
{
	...
	int xxx,yyy;
	if (((cxsj/100)!=0)&&((cxsj/100)%30==0)&&(xysj[(cxsj/100)/30]==false)&&((cxsj/100)/30>0)&&(xyz==0));
	{
	...
	switch(sl)
	{
		...
		case 72:{
			...
			if ((i2>1)&&(dt[i2-1][j2]!="█")&&((i2-1!=i2)||(j2!=j1))) {i2--;draw();}
			break;
		}
		case 75:{
			if (hly) 
...
void wu() //英雄详情
{
	...
	switch(sl)
	{
		case '1':{
			...
			cout<<"B键释放地雷埋于脚下，只对敌人生效，并造成200点基础伤害+100点随机伤害（效果可叠加）"<<endl;
			cout<<"N键当敌人于自己同一竖行或同一纵列时，突进将敌人逼入墙角，并且敌人每移动一格造成随机50点伤害"<<endl;
			cout<<"G键使用大招，10秒内获得隐身效果，且持续恢复自身血量（仍受血量上限的限制）"<<endl;
			cout<<"【回车返回】"<<endl;
			getchar();
			break;
		}
		case '2':{
			...
			cout<<"2键瞬移至敌人周围任意一格，虽不造成伤害，但这一技能更促使了其神一般的存在"<<endl;
			cout<<"3键开启霸体，如果敌方在自己所在九宫格周围的任意一格，将对敌人造成100点基础伤害+200点随机伤害,并将敌方弹至角落"<<endl;
			cout<<"5键使用大招，10秒内获得反伤护甲，期间仅损失少量血量，自己所受的攻击将反弹给敌人100%的真实伤害"<<endl;
			cout<<"【回车返回】"<<endl;
			getchar();
			break;
		}
...
void liu() //操作指南 
{
	...
	switch(sl)
	{
		case '1':{
			...
			cout<<"※为地雷爆炸后遗留的火源区，双方经过时都会减少血量；卍为幸运之神的礼物，走过可拾取并获得随机buff加成；"<<endl;
			cout<<"【回车返回】";
			getchar();
			break;
		}
...
int main()
{
	...
	////初始化 
	for (int i=1;i<=136;i++)
	{int x=i%8;Sleep(0.9);system("cls");printf("正在进入方块世界~    Loading···\n");for (int j=1;j<=i/8;j++) printf("█");cout<<jz[x];}
	Sleep(500);
	zjm();
	return 0;
}
